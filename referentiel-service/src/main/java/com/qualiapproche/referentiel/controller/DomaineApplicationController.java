...
public class DomaineApplicationController {
    ...
    @GetMapping("/all")
    public ResponseEntity<com.qualiapproche.common.response.ApiResponse<List<DomaineApplicationDto>>> all() {
        return ResponseEntity.ok(com.qualiapproche.common.response.ApiResponse.success(service.getAll()));
    }

    @PreAuthorize("@perm.canRead(this)")
    @GetMapping
    public ResponseEntity<Page<DomaineApplicationDto>> page(@ParameterObject Pageable pageable) {
